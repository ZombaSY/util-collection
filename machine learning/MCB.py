...
class MCBTest(nn.Module):

    def __init__(self):
        super(MCBTest, self).__init__()

        self.mcb_layer = CompactBilinearPooling(2048, 2048, 2048).double().cuda()
...
def main():
    ...
    x1 = torch.rand(4, 2048).double().cuda().requires_grad_()
    x2 = torch.rand(4, 2048).double().cuda().requires_grad_()

    y = torch.rand(4, 2048).double().cuda()
